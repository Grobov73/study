...
// Thu Mar 30 19:27:36 2017 by ROOT version 5.34/36
// from TTree cbmsim_reduced/cbmsim_reduced
// found on file: /home/evrazhka/mpddst_reduced_9845397_838.root
//////////////////////////////////////////////////////////
...
public :
   ...
   Float_t         eta_mc[1493];   //[n_tracks_mc]
   Float_t         pt_mc[1493];   //[n_tracks_mc]
   Int_t           mother_ID_mc[1493];   //[n_tracks_mc]
   Int_t           PDG_code_mc[1493];   //[n_tracks_mc]
   Float_t         px_mc[1493];   //[n_tracks_mc]
   Float_t         py_mc[1493];   //[n_tracks_mc]
   Float_t         pz_mc[1493];   //[n_tracks_mc]
   Float_t         start_x_mc[1493];   //[n_tracks_mc]
   Float_t         start_y_mc[1493];   //[n_tracks_mc]
   Float_t         start_z_mc[1493];   //[n_tracks_mc]
   Float_t         mass_mc[1493];   //[n_tracks_mc]
   Float_t         energy_mc[1493];   //[n_tracks_mc]
   Long64_t        n_tracks_mpd;
   Float_t         eta_mpd[698];   //[n_tracks_mpd]
   Float_t         phi_mpd[698];   //[n_tracks_mpd]
   Float_t         theta_mpd[698];   //[n_tracks_mpd]
   Int_t           TOF_flag_mpd[698];   //[n_tracks_mpd]
   Float_t         ZDC_energy_mpd[90];
   Float_t         pid_tpc_prob_electron_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_pion_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_kaon_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_proton_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_electron_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_pion_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_kaon_mpd[698];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_proton_mpd[698];   //[n_tracks_mpd]
   Float_t         tof_beta_mpd[698];   //[n_tracks_mpd]
   Float_t         tof_mass2_mpd[698];   //[n_tracks_mpd]
   Float_t         dEdx_tpc_mpd[698];   //[n_tracks_mpd]
   Float_t         chi2_mpd[698];   //[n_tracks_mpd]
   Float_t         pt_error_mpd[698];   //[n_tracks_mpd]
   Float_t         theta_error_mpd[698];   //[n_tracks_mpd]
   Float_t         phi_error_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_x_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_y_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_z_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_global_x_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_global_y_mpd[698];   //[n_tracks_mpd]
   Float_t         DCA_global_z_mpd[698];   //[n_tracks_mpd]
   Int_t           n_hits_mpd[698];   //[n_tracks_mpd]
   Int_t           n_hits_poss_mpd[698];   //[n_tracks_mpd]
   Float_t         signed_pt_mpd[698];   //[n_tracks_mpd]
   Long64_t        id_from_mc_mpd[698];   //[n_tracks_mpd]
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/home/evrazhka/mpddst_reduced_9845397_838.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/home/evrazhka/mpddst_reduced_9845397_838.root");
      }
      f->GetObject("cbmsim_reduced",tree);

   }
   Init(tree);
   h_pt_mpd = new TH1F("h_pt_mpd","proton transverse momentum mpd",50,0.0,4.0);h_pt_mpd->Sumw2();
   h_pt_mc = new TH1F("h_pt_mc","proton transverse momentum mc",50,0.0,4.0);h_pt_mc->Sumw2();
   h_p_eff = new TH1F("h_p_eff","efficiency",50,0.0,4.0); h_p_eff->Sumw2();

   h_kaon_pt_mpd = new TH1F("h_kaon_pt_mpd","kaon transverse momentum mpd",50,0.0,4.0);h_kaon_pt_mpd->Sumw2();
   h_kaon_pt_mc = new TH1F("h_kaon_pt_mc","kaon transverse momentum mc",50,0.0,4.0);h_kaon_pt_mc->Sumw2();
   h_k_eff = new TH1F("h_k_eff","efficiency",50,0.0,4.0); h_k_eff->Sumw2();

   h_pion_pt_mpd = new TH1F("h_pion_pt_mpd","pion transverse momentum mpd",50,0.0,4.0);h_pion_pt_mpd->Sumw2();
   h_pion_pt_mc = new TH1F("h_pion_pt_mc","pion transverse momentum mc",50,0.0,4.0);h_pion_pt_mc->Sumw2();
   h_pi_eff = new TH1F("h_pi_eff","efficiency",50,0.0,4.0); h_pi_eff->Sumw2();

   h_p_diff = new TH1F("h_p_diff", "difference of reco and mc momenta", 50,0.0,4.0);h_p_diff->Sumw2();
   h_ptcorr = new TH2F("h_ptcorr","reconstructed and mc momenta for protons",100, 0,2.5,50,0,2.5);

   h_p_eta_mpd = new TH1F("h_p_eta_mpd","proton eta mpd",50,0.0,4.0);h_p_eta_mpd->Sumw2();
   h_p_eta_mc = new TH1F("h_p_eta_mc","proton eta mc",50,0.0,4.0);h_p_eta_mc->Sumw2();
   h_peta_eff = new TH1F("h_peta_eff","efficiency",50,0.0,4.0); h_peta_eff->Sumw2();

   h_kaon_eta_mpd = new TH1F("h_kaon_eta_mpd","kaon eta mpd",50,0.0,4.0);h_kaon_eta_mpd->Sumw2();
   h_kaon_eta_mc = new TH1F("h_kaon_eta_mc","kaon eta mc",50,0.0,4.0);h_kaon_eta_mc->Sumw2();
   h_keta_eff = new TH1F("h_keta_eff","efficiency",50,0.0,4.0); h_keta_eff->Sumw2();

   h_pion_eta_mpd = new TH1F("h_pion_eta_mpd","pion eta mpd",50,0.0,4.0);h_pion_eta_mpd->Sumw2();
   h_pion_eta_mc = new TH1F("h_pion_eta_mc","pion eta mc",50,0.0,4.0);h_pion_eta_mc->Sumw2();
   h_pieta_eff = new TH1F("h_pieta_eff","eta efficiency",50,0.0,4.0); h_pieta_eff->Sumw2();
